...
_ANALYSIS_PROMPT_TEMPLATE = """계약서 조항의 **실질적 내용**을 분석하여 리스크를 평가해주세요.

[중요 지침]
- 조항 번호(제N조)나 제목만으로 판단하지 마세요. 반드시 조항 본문의 구체적 내용(의무, 권리, 조건, 금액, 기간 등)을 근거로 판단하세요.
- 본문 내용이 없고 제목/번호만 있는 경우: risk_level을 "LOW", confidence를 0.3으로 설정하고 summary에 "본문 내용을 확인할 수 없어 정확한 판단이 어렵습니다."라고 명시하세요.
- 손해배상 한도, 일방적 권리, 불균형한 의무 부담, 모호한 표현 등 계약 당사자에게 불리한 실질적 내용을 중점 분석하세요.
...
{{
  ...
  "summary": "한국어 리스크 요약 (2-3문장) — 조항 본문의 구체적 내용을 인용하여 작성",
  "rationale": "판단 근거 상세 설명 — 조항 내 특정 문구를 인용하고, 관련 판례/법령이 제공된 경우 이를 구체적으로 인용할 것"
}}

confidence 필드 설명:
- 0.0~1.0 사이의 숫자로, 위험도 판단에 대한 신뢰도를 나타냅니다.
- 1.0에 가까울수록 명확하고 확실한 판단, 0.5에 가까울수록 해석이 불분명합니다.
- 본문 없이 제목만 있는 경우 → 0.3, 명확한 고위험 조항 → 0.95, 해석 여지가 있는 조항 → 0.65
